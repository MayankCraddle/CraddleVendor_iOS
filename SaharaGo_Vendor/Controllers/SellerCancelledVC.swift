...
class SellerCancelledVC: UIViewController {

    @IBOutlet weak var cancelledTableView: UITableView!
    @IBOutlet weak var emptyView: UIView!
    @IBOutlet weak var mainView: UIView!
    
    var cancelledOrdersArr = [SellerOrdersStruct]()
    var pageNo = 1
    ...
    override func viewDidLoad() {
        ...
        self.cancelledTableView.register(UINib(nibName: "SellerCancelledCell", bundle: nil), forCellReuseIdentifier: "SellerCancelledCell")
    ...
    func getCancelledOrders() {
        if Reachability.isConnectedToNetwork() {
            ...
            let apiUrl = BASE_URL + PROJECT_URL.VENDOR_GET_CANCELLED_ORDER + "?pageNumber=\(self.pageNo)" + "&limit=15"
            ...
            ServerClass.sharedInstance.getRequestWithUrlParameters(param, path: urlStr as String, successBlock: { (json) in
                ...
                {
                    if self.pageNo == 1 {
                        self.cancelledOrdersArr.removeAll()
                    }
                    ...
                    if self.cancelledOrdersArr.count > 0 {
                        self.cancelledTableView.isHidden = false
                        self.emptyView.isHidden = true
                    } else {
                        self.cancelledTableView.isHidden = true
...
extension SellerCancelledVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.cancelledOrdersArr.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SellerCancelledCell", for: indexPath) as! SellerCancelledCell
        cell.selectionStyle = .none
        cell.threeDotsButton.isHidden = true
        cell.detailButton.isHidden = false
        
        let info = self.cancelledOrdersArr[indexPath.row]
...
//        cell.itemCurrencyPriceQquantityLbl.text = "\(info.currency) \(info.totalPrice) | \(info.quantity) Item"
        ...
        if indexPath.row == self.cancelledOrdersArr.count - 1 { // last cell
            if info.totalSize > self.cancelledOrdersArr.count { // more items to fetch
                self.pageNo += 1
                self.getCancelledOrders()
            }
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        sellerOrderDetailVC.orderDetailArr = cancelledOrdersArr[indexPath.row]
        sellerOrderDetailVC.isFrom = "CancelledOrder"
